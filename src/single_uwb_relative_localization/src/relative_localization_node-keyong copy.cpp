...
void* g2o_process(void* args)
{
	while(1)
	{
		...
		if(robot1_lab != 0 && robot1_lab > old_robot1_lab)
		{
			m_mtx_odom_est.lock();
			get_odom1_flag = 1;
			est_current_odom1_g2o.x = record_data["drone1"][robot1_lab].x;
			est_current_odom1_g2o.y = record_data["drone1"][robot1_lab].y;
			est_current_odom1_g2o.yaw = carmen_normalize_theta(record_data["drone1"][robot1_lab].theta_z);
			m_mtx_odom_est.unlock();
#ifdef PUB_G2O	
			nav_msgs::Odometry est_pose_;
		...
		if(robot2_lab != 0 && robot2_lab > old_robot2_lab)
		{
			m_mtx_odom_est.lock();
			get_odom2_flag = 1;
			est_current_odom2_g2o.x = record_data["drone2"][robot2_lab].x;
			est_current_odom2_g2o.y = record_data["drone2"][robot2_lab].y;
			est_current_odom2_g2o.yaw = carmen_normalize_theta(record_data["drone2"][robot2_lab].theta_z);
			m_mtx_odom_est.unlock();
#ifdef PUB_G2O	
			nav_msgs::Odometry est_pose_;
			est_pose_.header.stamp = ros::Time::now();
		...
		if(robot3_lab != 0 && robot3_lab > old_robot3_lab)
		{
			m_mtx_odom_est.lock();
			get_odom3_flag = 1;
			est_current_odom3_g2o.x = record_data["drone3"][robot3_lab].x;
			est_current_odom3_g2o.y = record_data["drone3"][robot3_lab].y;
			est_current_odom3_g2o.yaw = carmen_normalize_theta(record_data["drone3"][robot3_lab].theta_z);
			m_mtx_odom_est.unlock();
#ifdef PUB_G2O	
			nav_msgs::Odometry est_pose_;
			est_pose_.header.stamp = ros::Time::now();
