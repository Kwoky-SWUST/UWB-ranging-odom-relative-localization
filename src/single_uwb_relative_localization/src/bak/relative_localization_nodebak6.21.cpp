...
const double FINAL_TIME_WINDOW_SIZE = 100;
...
void plotEstGlo(int minX=-8, int maxX=15, int minY=-8, int maxY=20)
{
	std::string cmd;
...
double m_dBinningMinX=-5;
double m_dBinningMaxX=10;
double m_dBinningMinY=-8;
double m_dBinningMaxY=10;
// --------------------------------------------------------------------------
						   ...
						   boost::circular_buffer<UWBMessage> &v_robot_uwb_measure)
{
	...
	{	
		...
		if(total_num-TIME_WINDOW_SIZE > TIME_GRAP  && TIME_WINDOW_SIZE <= FINAL_TIME_WINDOW_SIZE)
		{
		...
		if(total_num > TIME_WINDOW_SIZE)
		{
			...
			if(anchor_name+tag_name=="robot1robot2")
			{
			...
			if(anchor_name+tag_name=="robot1robot3")
			{
				nav_msgs::Odometry rel_pose;
				rel_pose.pose.pose.orientation.x = time;
				rel_pose.pose.pose.position.x = relative_pose.x;
...
void robot1OdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
	...
	m_v_odom_pose["drone1"].push_back(pose);
	m_v_est_globoal_pose["robot1"].push_back(est_pose);
	std::ofstream odom_file(drone_odom_file1, std::ios::app);
	odom_file << "drone1" << "\t" << to_string(pose.timestamp) << "\t" << to_string(pose.x) << "\t" << to_string(pose.y) << "\t" << to_string(pose.yaw) << "\n";
	odom_file.close();
}
//------------------------------------------------------------------------
void robot2OdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
	...
	m_v_odom_pose["drone2"].push_back(pose);
	m_v_est_globoal_pose["robot2"].push_back(est_pose);
	std::ofstream odom_file(drone_odom_file2, std::ios::app);
...
void robot3OdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
	...
	m_v_odom_pose["drone3"].push_back(pose);
	m_v_est_globoal_pose["robot3"].push_back(est_pose);
	std::ofstream odom_file(drone_odom_file3, std::ios::app);
	odom_file << "drone3" << "\t" << to_string(pose.timestamp) << "\t" << to_string(pose.x) << "\t" << to_string(pose.y) << "\t" << to_string(pose.yaw) << "\n";
	odom_file.close();
}
//------------------------------------------------------------------------
...
// amcl
		std::map<std::string, std::vector<RobotPose> >::iterator it_amcl;
		it_amcl = m_v_amcl.find(anchor_name);
	...
	// cout << to_string(drone1_current_amcl.timestamp) << "  " <<  drone1_current_amcl.x << "   " << drone1_current_amcl.y << "  " << drone1_current_amcl.yaw << endl;
		it_amcl = m_v_amcl.find(tag_name);
...
void* g2o_process(void* args)
{
	while(1)
	{
		...
		if(m_v_odom_pose.size() < 3 || m_v_robot_pose.size() < 2 || m_v_rel_pose.size() < 2)
			continue;
		...
		// add drone1 and drone2 vexter
		std::map< std::string, std::vector<RobotPose> >::iterator it_m_drone_position=m_v_robot_pose.find("drone2");
		std::vector<RobotPose> drone_position_vecter=it_m_drone_position->second;
		for(int i=0; i<drone_position_vecter.size(); i++)
		{
			...
			//add robot1 vexter
			std::map< std::string, std::vector<RobotPose> >::iterator it_odom=m_v_odom_pose.find("drone1");
			std::vector<RobotPose> it_odom_vecter=it_odom->second;
			double timestamp_temp=INF;
			int t_num=0;
			for(int t=0; t<it_odom_vecter.size(); t++)
			{
				RobotPose drone1_odom_pose=it_odom_vecter[t];
				double drone1_timestamp=drone1_odom_pose.timestamp;
				double diff_time=fabs(drone1_timestamp-drone_timestamp);
				if(diff_time < timestamp_temp)
				{
					timestamp_temp=diff_time;
					t_num=t;
				}
			}
			if(timestamp_temp > time_limit)
	...
	if(old_t_num == t_num)
		continue;
	old_t_num = t_num;
			double drone1_time=it_odom_vecter[t_num].timestamp;
			double drone1_x=it_odom_vecter[t_num].x;
			double drone1_y=it_odom_vecter[t_num].y;
			double drone1_theta=it_odom_vecter[t_num].yaw;

			RobotPose drone1_pose;
			drone1_pose.x=drone1_x;
			drone1_pose.y=drone1_y;
			drone1_pose.yaw=drone1_theta;
			drone1_pose.timestamp=drone1_time;

			drone1_vexter_vecter.push_back(drone1_pose);

			Eigen::Quaterniond drone1_q;
			Eigen::AngleAxisd drone1_rollAngle(AngleAxisd(0,Vector3d::UnitX()));
			Eigen::AngleAxisd drone1_pitchAngle(AngleAxisd(0,Vector3d::UnitY()));
			Eigen::AngleAxisd drone1_yawAngle(AngleAxisd(drone1_theta,Vector3d::UnitZ()));

			drone1_q=drone1_yawAngle*drone1_pitchAngle*drone1_rollAngle;

			Eigen::Vector3d drone1_trans(drone1_x,drone1_y,0);
			g2o::SE3Quat drone1_current(drone1_q,drone1_trans);

			VertexSE3* robot_1 =  new VertexSE3;

			robot_1->setId(robot1_index);
			robot_1->setEstimate(drone1_current);
		...
		// if(vexter_num3 < 20 || vexter_num12<20)
		//  continue;
		...
		for(it_odom_egde=m_v_odom_pose.begin(); it_odom_egde!=m_v_odom_pose.end(); ++it_odom_egde)
		{
			...
			if(drone_name == "drone2")
			{
				...
				for(int i=0; i< drone1_vexter_vecter.size(); i++)
				{
					...
					double timestamp_temp=INF;
					double t_num=0;
					for(int t=0; t<odom_vector.size(); t++)
					{
						double time_diff=fabs(drone1_time - odom_vector[t].timestamp);
						if(time_diff < timestamp_temp)
						{
							timestamp_temp=time_diff;
							t_num=t;
					...
					RobotPose drone_pose=odom_vector[t_num];
			...
			if(drone_name == "drone3")
			{
				...
				for(int i=0; i< drone_position_vecter.size(); i++)
				{
					RobotPose drone1_pose=drone_position_vecter[i];
	...
	old_t_num2 = t_num2;
		...
		std::map <std::string, std::map <double, carmen_6d_point_t>> record_data;
		...
		for(it_map = graph_vertices.begin(); it_map != graph_vertices.end(); ++it_map)
		{
			...
			robot_pose.theta_z = robot_yaw;
			
			std::string drone;
			double timestamp_num=0;
		...
		std::map <std::string, std::map <double, carmen_6d_point_t>>::iterator it_record;
		for(it_record=record_data.begin(); it_record!=record_data.end(); ++it_record)
		{
			std::string drone_name=it_record->first;

			std::map <double, carmen_6d_point_t> data=it_record->second;
			if(drone_name == "drone3")
			{
				for(int i = 0; i < drone_position_vecter.size(); i++)
				{
					std::map <double, carmen_6d_point_t>::iterator it_data=data.find(double(i));
					...
					adjusted_data[drone_name][double(i)]=adjusted;
			...
			plot_all_g2o_tracks();
			// cout << to_string(m_g2o_results["drone1"].back().timestamp)  << "\t" << to_string(m_g2o_results["drone2"].back().timestamp) << "\t" << to_string(m_g2o_results["drone3"].back().timestamp) << endl;
			est_current_odom1 = m_g2o_results["drone1"].back();
			est_current_odom2 = m_g2o_results["drone2"].back();
			est_current_odom3 = m_g2o_results["drone3"].back();
			m_g2o_results.clear();
	
	
	// // compute error
	// 	// Final: Compute Error of relative pose
	// 	std::cout << "Summary: \n";
	// 	std::cout << "Vexter: " << robot1_index << "   " << robot2_index << "  " << robot3_index << "   " << vexter_num12 << "  " << vexter_num3 << std::endl;
	// 	std::cout << "Edge: " << robot12_edge << "   " << robot13_edge << "  " << robot23_edge << "   " <<  std::endl;
...
int main(int argc, char *argv[]){
	...
	robot2_pub = n.advertise<nav_msgs::Odometry>("/robot2/relative_pose", 10);
	robot3_pub = n.advertise<nav_msgs::Odometry>("/robot3/relative_pose", 10);
