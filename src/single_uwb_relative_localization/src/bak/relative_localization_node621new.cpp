...
const double FINAL_TIME_WINDOW_SIZE = 80;
...
void* g2o_process(void* args)
{
	while(1)
	{
		...
		if(m_v_odom_pose.size() < 2 || m_v_robot_pose.size() < 3 || m_v_rel_pose.size() < 2)
			continue;
	...
	std::vector<RobotPose> drone1_vexter_vecter=it_m_drone_position->second;
...
old_t_num2 = t_num2;
		...
		static RobotPose old_est_current_odom1;
		static RobotPose old_est_current_odom2;
		static RobotPose old_est_current_odom3;
		static int flag = 1;
		if(flag)
		{
			old_est_current_odom1 = m_g2o_results["drone1"].back();
			old_est_current_odom2 = m_g2o_results["drone2"].back();
			old_est_current_odom3 = m_g2o_results["drone3"].back();
			flag = 0;
		}
		else
		{
			double dist1=sqrt((old_est_current_odom1.x-est_current_odom1.x)*(old_est_current_odom1.x-est_current_odom1.x)
							+(old_est_current_odom1.y-est_current_odom1.y)*(old_est_current_odom1.y-est_current_odom1.y));
			double dist2=sqrt((old_est_current_odom2.x-est_current_odom2.x)*(old_est_current_odom2.x-est_current_odom2.x)
							+(old_est_current_odom2.y-est_current_odom2.y)*(old_est_current_odom2.y-est_current_odom2.y));
			double dist3=sqrt((old_est_current_odom3.x-est_current_odom3.x)*(old_est_current_odom3.x-est_current_odom3.x)
							+(old_est_current_odom3.y-est_current_odom3.y)*(old_est_current_odom3.y-est_current_odom3.y));
			...
			{
				old_est_current_odom1 = est_current_odom1;
			}
			if(dist2 <= 0.4)
			{
			...
			{
				old_est_current_odom2 = est_current_odom2;
			}
			if(dist3 <= 0.4)
			{
			...
			{
				old_est_current_odom3 = est_current_odom3;
			}
			g2o_flag = 1;
		}
