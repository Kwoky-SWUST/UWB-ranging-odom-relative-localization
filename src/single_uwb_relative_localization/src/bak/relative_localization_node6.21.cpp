...
const int LAMDA = 20;
const double UPDATE_DIST = 0.02;  // m
const double UPDATE_ANGLE = 0.02; // m
const double UPDATE_TIME = 0.5;  // s
const double FINAL_TIME_WINDOW_SIZE = 800;
...
void* poseGraph_process(void* args)
{
	while(ros::ok())
	{
		...
		// 			RobotPose odom = computeT1inverse_x_T2(old_odom_edge, odom_edge);
		// 			g2o::SE3Quat odom_edge_se3 = computePoseToG2oSE3(odom);
		...
		if(vertex2_num*vertex3_num != 0)
		{
			...
			for(it_map = graph_vertices.begin(); it_map != graph_vertices.end(); ++it_map)
			{
				...
				if(it_map->first >= vertex1_num && it_map->first < (vertex1_num+vertex2_num))
				{
			    ...
			    if(it_map->first >= (vertex1_num+vertex2_num) && it_map->first < (vertex1_num+vertex2_num+vertex3_num))
				{
	...
	{	
		...
		if(total_num > TIME_WINDOW_SIZE)
		{
			...
			if(anchor_name+tag_name=="robot1robot2")
			{
				...
				RobotPose robot_pose = computeT1_x_T2(now_odom.anchorPose, relative_pose);
				g2o::SE3Quat rel_inver_se3 = computePoseToG2oSE3(relative_pose).inverse();
				RobotPose rel_inver_pose = computeG2oSE3ToPose(rel_inver_se3);
				RobotPose robot_anchor_pose = computeT1_x_T2(now_odom.tagPose, rel_inver_pose);
				m_v_vertex["robot1"].push_back(robot_anchor_pose);
				m_v_vertex["robot2"].push_back(robot_pose);
				m_v_odom_edge["robot1"].push_back(now_odom.anchorPose);
				m_v_odom_edge["robot2"].push_back(now_odom.tagPose);
			}
			if(anchor_name+tag_name=="robot1robot3")
			{
				...
				RobotPose robot_pose = computeT1_x_T2(now_odom.anchorPose, relative_pose);
				m_v_vertex["robot3"].push_back(robot_pose);
				m_v_odom_edge["robot3"].push_back(now_odom.tagPose);
			}
		}
...
void robot1AMCLCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
	...
	{
		RobotPose relative_pose = computeT1inverse_x_T2(robot_gt, m_v_amcl["robot2"].back());
		nav_msgs::Odometry rel_pose;
...
void robot2AMCLCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
	...
	{
		RobotPose relative_pose = computeT1inverse_x_T2(robot_gt, m_v_amcl["robot1"].back());
		nav_msgs::Odometry rel_pose;
		rel_pose.pose.pose.orientation.x = timestamp;
		rel_pose.pose.pose.position.x = relative_pose.x;
		rel_pose.pose.pose.position.y = relative_pose.y;
...
int main(int argc, char *argv[]){
	...
	pthread_t tid1;
	int ret1 = pthread_create(&tid1, NULL, poseGraph_process, NULL);
	ros::spin();
	return 0;
}
