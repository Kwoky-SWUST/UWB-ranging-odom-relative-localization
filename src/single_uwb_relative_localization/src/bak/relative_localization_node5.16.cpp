...
void plotAMCL(int minX=-10, int maxX=10, int minY=-10, int maxY=10)
{
	std::string cmd;
						   ...
						   boost::circular_buffer<UWBMessage> &v_robot_uwb_measure2)
{
	...
	if(m_flag[anchor_name]==1 && fabs(m_v_amcl[anchor_name].back().timestamp  - time) >= 0.1)
	{
		...
		if(cnt1 != 0)
		{
			...
			if(timeSynchronization(anchor_name, tag_1_name, now_amcl, now_odom))
			{
				...
				if(total_num > TIME_WINDOW_SIZE)
				{
					...
					m_rel_pose[anchor_name+tag_1_name] = relative_pose;
				}
				ros::Time end = ros::Time::now();
		...
		if(cnt2 != 0)
		{
			...
			CommonPose now_amcl;
			CommonPose now_odom;
			if(timeSynchronization(anchor_name, tag_2_name, now_amcl, now_odom))
			{
				RobotPose est_rel;
				...
				if(total_num > TIME_WINDOW_SIZE)
				{
					...
					m_rel_pose[anchor_name+tag_2_name] = relative_pose;
				}
				ros::Time end = ros::Time::now();
...
void robot1AMCLCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
	...
	{
		g2o::SE3Quat rel = computePoseToG2oSE3(robot_gt).inverse() * computePoseToG2oSE3(m_v_amcl["robot2"].back());
		m_rel_pose["robot1robot2"] = computeG2oSE3ToPose(rel);
	}
		plotAMCL();
	// RobotPose pose1;
	// pose1.x = 0;
...
void robot2AMCLCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
	...
	{
		g2o::SE3Quat rel = computePoseToG2oSE3(m_v_amcl["robot1"].back()) * computePoseToG2oSE3(robot_gt).inverse();
		m_rel_pose["robot1robot2"] = computeG2oSE3ToPose(rel);
	}
...
int main(int argc, char *argv[]){
	...
	ros::Publisher robot2_pub = n.advertise<geometry_msgs::Point>("/robot2/relative_pose", 10);
	ros::Publisher robot3_pub = n.advertise<geometry_msgs::Point>("/robot3/relative_pose", 10);
	// pthread_t tid;
