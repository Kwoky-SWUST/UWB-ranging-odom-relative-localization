...
const int TIME_WINDOW_SIZE = 40;
const int TIME_GRAP = 2;
						   ...
						   boost::circular_buffer<UWBMessage> &v_robot_uwb_measure2)
{
	...
	if(visual <= 0.02)
	{
		...
		int cnt2 = m_v_win_uwb[anchor_name+tag_2_name].size();
...
int main(int argc, char *argv[]){
	...
	pthread_t tid;
	int ret = pthread_create(&tid, NULL, poseGraph_process, NULL);
	ros::spin();
	return 0;
}
