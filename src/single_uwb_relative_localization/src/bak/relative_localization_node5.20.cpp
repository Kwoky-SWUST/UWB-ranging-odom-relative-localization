...
const int TIME_WINDOW_SIZE = 500;
const int TIME_GRAP = 30;
...
bool timeSynchronization(std::string anchor_name, std::string tag_name, CommonPose &now_odom)
{
	...
	now_odom.tagPose.timestamp = m_v_odom[tag_name][record_i].timestamp;
...
{
	if(fabs(m_v_odom[anchor_name].back().timestamp  - time) < 0.03)
	{
	...
	if(m_flag[anchor_name]==1 && fabs(m_v_odom[anchor_name].back().timestamp  - time) >= 0.03)
	{
			...
			if(timeSynchronization(anchor_name, tag_1_name, now_odom))
			{
				...
				if(total_num > TIME_WINDOW_SIZE)
				{
					...
					if(anchor_name+tag_1_name=="robot1robot2")
					{
						...
						// rel_pose.x = relative_pose.x;
						// rel_pose.y = relative_pose.y;
						// rel_pose.z = relative_pose.yaw;
						// robot2_pub.publish(rel_pose);
					}
					// m_rel_pose[anchor_name+tag_1_name] = relative_pose;
					cout << relative_pose.x << "\t" << relative_pose.y << "\t" << relative_pose.yaw << endl;
				}
				ros::Time end = ros::Time::now();
		...
		if(cnt2 != 0)
		{
			...
			CommonPose now_odom;
			if(timeSynchronization(anchor_name, tag_2_name, now_odom))
			{
				RobotPose est_rel;
				...
				cout << anchor_name+tag_2_name  << ":\t" << m_v_com_odom[anchor_name+tag_2_name].size() << "\t" << v_robot_uwb_measure2.size() << "\t" << endl;
				int total_num = m_v_com_odom[anchor_name+tag_2_name].size();
				RobotPose relative_pose;
				ros::Time begin_g2o = ros::Time::now();
				if(total_num > TIME_WINDOW_SIZE)
				{
					relative_pose = nonlinearOptimization_process(total_num, anchor_name, tag_2_name, m_v_com_odom[anchor_name+tag_2_name], v_robot_uwb_measure2);
				}
				ros::Time end = ros::Time::now();
...
{
	if(m_v_odom["robot1"].size() <= 0) return;
...
{
	if(m_v_odom["robot2"].size() <= 0) return;
	double time = fabs(msg->sec + (double)(msg->usec)/1000000.0 - firstTimestamp);
	int uwb_id = msg->id;
...
int main(int argc, char *argv[]){
	...
	robot2_pub = n.advertise<geometry_msgs::Point>("/robot2/relative_pose", 1);
