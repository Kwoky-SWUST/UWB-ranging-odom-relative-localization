...
bool timeSynchronization(std::string anchor_name, std::string tag_name, CommonPose &now_amcl, CommonPose &now_odom)
{
		...
		record_i=-1;
		for(int i=old_i[1]; i<m_v_odom[anchor_name].size(); i++)
		{
		...
		time_min_diff = INFINITY;
		record_i=-1;
		for(int i=old_i[2]; i<m_v_odom[tag_name].size(); i++)
		{
...
void uwbKalmanFilter(UWBMessage &uwb_message, CommonPose now_odom, RobotPose est_rel)
{
...
void prepareOptimationData(std::string anchor_name, std::string tag_1_name, std::string tag_2_name, double time, std::map<int, UWBMessage> m_node)
{
	...
	static int cnt1 = 0;
	static int cnt2 = 0;
	static std::map<std::string, UWBMessage> m_sum_uwb;
	if(fabs(m_v_amcl[anchor_name].back().timestamp  - time) < 0.1)
	{
		...
		for(it_node=m_node.begin(); it_node!=m_node.end(); it_node++)
		{
			...
			{
				m_sum_uwb[tag_1_name].range += it_node->second.range;
				m_sum_uwb[tag_1_name].fpRssi += it_node->second.fpRssi;
				m_sum_uwb[tag_1_name].rxRssi += it_node->second.rxRssi;
				cnt1++;
				// std::ofstream uwb_file(uwb_measure_name, std::ios::app);
				// uwb_file << "robot1 robot2" << "\t" << to_string(time) << "\t" << to_string( msg->node[i].dis) << "\t" << to_string(msg->node[i].rxRssi) << "\t" << to_string(msg->node[i].fpRssi) << "\n";
				// uwb_file.close();
			}
			else if(it_node->first == m_uwb_configure[tag_2_name].uwb_id)
			{

				m_sum_uwb[tag_2_name].range += it_node->second.range;
				m_sum_uwb[tag_2_name].fpRssi += it_node->second.fpRssi;
				m_sum_uwb[tag_2_name].rxRssi += it_node->second.rxRssi;
				cnt2++;
			}
		}
	}
	if(flag1 == 1 && fabs(m_v_amcl[anchor_name].back().timestamp  - time) >= 0.1)
	{
		flag1 = 0;
		...
		if(cnt1 != 0)
		{
			...
			uwb_message.range = m_sum_uwb[tag_1_name].range/(double)(cnt1);
			uwb_message.fpRssi = m_sum_uwb[tag_1_name].fpRssi/(double)(cnt1);
			uwb_message.rxRssi = m_sum_uwb[tag_1_name].rxRssi/(double)(cnt1);
			...
			if(timeSynchronization(anchor_name, tag_1_name, now_amcl, now_odom))
			{
				...
				uwbKalmanFilter(uwb_message, now_odom, est_rel);
				m_v_com_amcl[anchor_name+tag_1_name].push_back(now_amcl);
				m_v_com_odom[anchor_name+tag_1_name].push_back(now_odom);
				v_robot12_uwb_measure.push_back(uwb_message);
				cout << m_v_com_odom[anchor_name+tag_1_name].size() << "\t" << m_v_com_amcl[anchor_name+tag_1_name].size() << "\t" << v_robot12_uwb_measure.size() << "\t" << endl;
				...
				// double timestamp = uwb_message.timestamp;
				// std::ofstream amcl_file(com_amcl_file, std::ios::app);
				// amcl_file << to_string(timestamp) << "\t" << to_string(now_amcl.tagPose.x) << "\t" << to_string(now_amcl.tagPose.y) << "\t" << to_string(now_amcl.tagPose.yaw)
				// 		<< "\t" << to_string(now_amcl.anchorPose.x) << "\t" << to_string(now_amcl.anchorPose.y) << "\t" << to_string(now_amcl.anchorPose.yaw) << "\n";
				// amcl_file.close();
				// 	std::ofstream odom_file(com_odom_file, std::ios::app);
				// odom_file << to_string(timestamp) << "\t" << to_string(now_odom.tagPose.x) << "\t" << to_string(now_odom.tagPose.y) << "\t" << to_string(now_odom.tagPose.yaw)
				// 	<< "\t" << to_string(now_odom.anchorPose.x) << "\t" << to_string(now_odom.anchorPose.y) << "\t" << to_string(now_odom.anchorPose.yaw) << "\n";
				// odom_file.close();
				// std::ofstream uwb_file(com_uwb_measure_file, std::ios::app);
				// uwb_file << to_string(timestamp) << "\t" << to_string(uwb_message.range)
				// 	<< "\t" << to_string(uwb_message.rxRssi)
				// 	<< "\t" << to_string(uwb_message.fpRssi) << "\n";
				// uwb_file.close();
				// std::ofstream true_file(com_trues_dist_file, std::ios::app);
				// true_file << to_string(timestamp) << "\t" << to_string(true_dist) << "\n";
				// true_file.close();
				

				true_dist = sqrt((now_amcl.anchorPose.x-now_amcl.tagPose.x)*(now_amcl.anchorPose.x-now_amcl.tagPose.x)
		...
		m_sum_uwb[tag_1_name].rxRssi = m_sum_uwb[tag_1_name].fpRssi = m_sum_uwb[tag_1_name].range = 0;
		cnt1 = cnt2 = 0;		
	}
}
//========================================================================
void robot1UWBCallback(const nlink_parser::LinktrackNodeframe2::ConstPtr &msg)
{
	...
	// prepareOptimationData("robot1", "robot2", "robot3", time, m_node);
...
void robot2UWBCallback(const nlink_parser::LinktrackNodeframe2::ConstPtr &msg)
{
	...
	prepareOptimationData("robot2", "robot1", "robot3", time, m_node);
}
//------------------------------------------------------------------------
