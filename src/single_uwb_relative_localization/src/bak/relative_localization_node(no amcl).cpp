...
std::string gt1_file_name("/home/dzy/post-graduation_ws/src/single_uwb_relative_localization/data/gt_1_interpolation.txt");
std::string gt2_file_name("/home/dzy/post-graduation_ws/src/single_uwb_relative_localization/data/gt_2_interpolation.txt");
...
RobotPose nonlinearOptimization_process(int total_num, std::string anchor_name, std::string tag_name, std::vector<CommonPose> v_com_amcl, std::vector<CommonPose> v_com_odom, boost::circular_buffer<UWBMessage> v_robot_uwb_measure)
{  
	valid_count++;
	...
	{
		g2o::SE3Quat old_odom_se3 = computePoseToG2oSE3(m_old_odom[anchor_name+tag_name]);
	...
	cout << "non time cast:" << end.toSec()-begin.toSec() << "\t" << end_g2o.toSec() - begin_g2o.toSec() << endl;
...
void* computePositionError(void* args)
{
	...
	{	
		...
		std::cout << "robot1 to robot2" << "-poseError:" << mean_pose_error << "\t" << mean_pose_std << "\n"
			                        << "      -yawError:" << mean_yaw_error << "\t" << mean_yaw_std << std::endl;
		sleep(1);
	}
...
bool timeSynchronization(double time, std::string anchor_name, std::string tag_name, CommonPose &now_amcl, CommonPose &now_odom)
{
	std::map<std::string, IndexNum>::iterator it_index = m_index_record.find(anchor_name+tag_name);
	if(it_index == m_index_record.end())
		m_index_record[anchor_name+tag_name].index1 = m_index_record[anchor_name+tag_name].index2 = m_index_record[anchor_name+tag_name].index3 =m_index_record[anchor_name+tag_name].index4= 0;
	...
	record_i=-1;
	for(int i=m_index_record[anchor_name+tag_name].index2; i<m_v_amcl[anchor_name].size(); i++)
	{
		double time_diff = fabs(timestamp - m_v_amcl[anchor_name][i].timestamp);
		if(time_diff < time_min_diff)
		{
			time_min_diff = time_diff;
			record_i  = i;
	...
	if(time_min_diff > time_limit||record_i < 0)
		return false;
	now_amcl.anchorPose =  m_v_amcl[anchor_name][record_i];
	m_index_record[anchor_name+tag_name].index2 = record_i;

	time_min_diff = INFINITY;
	record_i=-1;
	for(int i=m_index_record[anchor_name+tag_name].index3; i<m_v_odom[anchor_name].size(); i++)
	{
	...
	time_min_diff = INFINITY;
	record_i=-1;
	for(int i=m_index_record[anchor_name+tag_name].index4; i<m_v_odom[tag_name].size(); i++)
	{
	...
	m_index_record[anchor_name+tag_name].index4 = record_i;
						   ...
						   boost::circular_buffer<UWBMessage> &v_robot_uwb_measure2)
{
	...
	UWBMessage robot2_uwb;
	UWBMessage robot3_uwb;
	for(it_node=m_node.begin(); it_node!=m_node.end(); it_node++)
	{
		if(it_node->first == m_uwb_configure[tag_1_name].uwb_id)
		{
			robot2_uwb.range = it_node->second.range;
			robot2_uwb.fpRssi = it_node->second.fpRssi;
			robot2_uwb.rxRssi = it_node->second.rxRssi;
		}
		else if(it_node->first == m_uwb_configure[tag_2_name].uwb_id)
		{
			robot3_uwb.range = it_node->second.range;
			robot3_uwb.fpRssi = it_node->second.fpRssi;
			robot3_uwb.rxRssi = it_node->second.rxRssi;
		}
	}		
	double visual = carmen_uniform_random(0, 1);
	if(visual <= 0.5)
	{
		...
		if(it_node->first == m_uwb_configure[tag_1_name].uwb_id)
		{
			...
			uwb_message.timestamp = time;
			uwb_message.range = robot2_uwb.range;
			uwb_message.fpRssi = robot2_uwb.fpRssi;
			uwb_message.rxRssi = robot2_uwb.rxRssi;
			CommonPose now_amcl;
			CommonPose now_odom;
			if(timeSynchronization(time, anchor_name, tag_1_name, now_amcl, now_odom))
			{
				RobotPose est_rel;
				...
				// plotComODOMAMCL(m_v_com_amcl[anchor_name+tag_1_name], m_v_com_odom[anchor_name+tag_1_name]);
				cout << anchor_name+tag_1_name  << ":\t" << m_v_com_odom[anchor_name+tag_1_name].size() << "\t" << m_v_com_amcl[anchor_name+tag_1_name].size() << "\t" << v_robot_uwb_measure1.size() << "\t" << endl;
				int total_num = m_v_com_amcl[anchor_name+tag_1_name].size();
				RobotPose relative_pose;
				ros::Time begin_g2o = ros::Time::now();
				if(total_num > TIME_WINDOW_SIZE)
				{
					relative_pose = nonlinearOptimization_process(total_num, anchor_name, tag_1_name, m_v_com_amcl[anchor_name+tag_1_name], m_v_com_odom[anchor_name+tag_1_name], v_robot_uwb_measure1);
					if(anchor_name == "robot1")
					{
						g2o::SE3Quat vertex_se3 = computePoseToG2oSE3(now_odom.anchorPose) * computePoseToG2oSE3(relative_pose);
						RobotPose vertex = computeG2oSE3ToPose(vertex_se3);
						m_v_vertex[tag_1_name].push_back(vertex);
						m_v_vertex[anchor_name].push_back(now_odom.anchorPose);
						m_v_odom_edge[tag_1_name].push_back(now_odom.tagPose);
						m_v_odom_edge[anchor_name].push_back(now_odom.anchorPose);
						v_uwb12_edge.push_back(relative_pose);
					}
				}
				ros::Time end = ros::Time::now();
				...
				saveComData(now_odom, now_amcl, uwb_message, true_dist);
			}
		}	
		// if(cnt2 != 0)
		// {
		...
		// 	{
		// 		sum.range += m_v_win_uwb[anchor_name+tag_2_name][i].range;
		// 		sum.fpRssi += m_v_win_uwb[anchor_name+tag_2_name][i].fpRssi;
		// 		sum.rxRssi += m_v_win_uwb[anchor_name+tag_2_name][i].rxRssi;
		// 	}
		// 	m_v_win_uwb[anchor_name+tag_2_name].clear();
		// 	UWBMessage uwb_message;
		// 	uwb_message.timestamp = m_v_amcl[anchor_name].back().timestamp;
		// 	uwb_message.range = sum.range/(double)(cnt2);
		// 	uwb_message.fpRssi = sum.fpRssi/(double)(cnt2);
		// 	uwb_message.rxRssi = sum.rxRssi/(double)(cnt2);

		// 	CommonPose now_amcl;
		// 	CommonPose now_odom;
		// 	if(timeSynchronization(time, anchor_name, tag_2_name, now_amcl, now_odom))
		// 	{
		// 		RobotPose est_rel;
		// 		std::map<std::string, RobotPose>::iterator it_est = m_est_rel.find(anchor_name+tag_2_name);
		// 		if(it_est == m_est_rel.end())
		// 			est_rel.timestamp = 666;
		// 		else
		// 		{
		// 			est_rel =  m_est_rel[anchor_name+tag_2_name];
		// 			est_rel.timestamp = 777;
		// 		}
		// 		// uwbKalmanFilter(uwb_message, now_odom, est_rel, kf2, anchor_name+tag_2_name);
				
		// 		m_v_com_amcl[anchor_name+tag_2_name].push_back(now_amcl);
		// 		m_v_com_odom[anchor_name+tag_2_name].push_back(now_odom);
		// 		v_robot_uwb_measure2.push_back(uwb_message);
		// 		// plotComODOMAMCL(m_v_com_amcl[anchor_name+tag_2_name], m_v_com_odom[anchor_name+tag_2_name]);
		// 		cout << anchor_name+tag_2_name  << ":\t" << m_v_com_odom[anchor_name+tag_2_name].size() << "\t" << m_v_com_amcl[anchor_name+tag_2_name].size() << "\t" << v_robot_uwb_measure2.size() << "\t" << endl;
		// 		int total_num = m_v_com_amcl[anchor_name+tag_2_name].size();
		// 		RobotPose relative_pose;
		// 		ros::Time begin_g2o = ros::Time::now();
		// 		if(total_num > TIME_WINDOW_SIZE)
		// 		{
		// 			relative_pose = nonlinearOptimization_process(total_num, anchor_name, tag_2_name, m_v_com_amcl[anchor_name+tag_2_name], m_v_com_odom[anchor_name+tag_2_name], v_robot_uwb_measure2);
		// 			if(anchor_name=="robot1")
		// 			{
		// 				g2o::SE3Quat vertex_se3 = computePoseToG2oSE3(now_odom.anchorPose) * computePoseToG2oSE3(relative_pose);
		// 				RobotPose vertex = computeG2oSE3ToPose(vertex_se3);
		// 				m_v_vertex[tag_2_name].push_back(vertex);
		// 				m_v_odom_edge[tag_2_name].push_back(now_odom.tagPose);
		// 				v_uwb13_edge.push_back(relative_pose);
		// 			}
		// 			if(anchor_name=="robot2")
		// 			{
		// 				v_uwb23_edge.push_back(relative_pose);
		// 			}
		// 		}
		// 		ros::Time end = ros::Time::now();
		...
		// 		double time_cast = end.toSec() - begin.toSec();
		// 		double g2o_time_cast = end.toSec() - begin_g2o.toSec();
		// 		cout << "data time_cast: " << time_cast << "\t" << g2o_time_cast << endl; 
		// 	}
		// }
	}		
}
...
int main(int argc, char *argv[]){
	...
	// loadAMCLFile(gt2_file_name);
	std::cout << m_v_amcl["robot1"].size() << "\t" << m_v_amcl["robot2"].size() << std::endl;
