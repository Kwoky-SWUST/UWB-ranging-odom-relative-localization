	...
	if(m_flag[anchor_name]==1 && fabs(m_v_amcl[anchor_name].back().timestamp  - time) >= 0.1)
	{
		...
		if(cnt1 != 0)
		{
			...
			if(timeSynchronization(anchor_name, tag_1_name, now_amcl, now_odom))
			{
				...
				{
					nonlinearOptimization_process(total_num, anchor_name, tag_1_name, m_v_com_amcl[anchor_name+tag_1_name], m_v_com_odom[anchor_name+tag_1_name], v_robot_uwb_measure1);
				}
		...
		if(cnt2 != 0)
		{
			...
			if(timeSynchronization(anchor_name, tag_2_name, now_amcl, now_odom))
			{
				...
				if(total_num > TIME_WINDOW_SIZE)
				{
					nonlinearOptimization_process(total_num, anchor_name, tag_2_name, m_v_com_amcl[anchor_name+tag_2_name], m_v_com_odom[anchor_name+tag_2_name], v_robot_uwb_measure2);
				}
				ros::Time end = ros::Time::now();
