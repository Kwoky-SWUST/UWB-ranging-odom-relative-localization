...
const double INITIAL3_X = -0.85;
const double INITIAL3_Y = -0.85;
...
double uwbKalmanFilter(UWBMessage uwb_message, CommonPose now_odom, RobotPose est_rel, KalmanFilter *kf, std::string name)
{
	...
	else if(fp > 0.6 && fp <= 5.0)
	    uwb_message.range = 0.122288*(0.6/(fp*2.3)) + 1.0013*uwb_message.range;
	...
	{	
		...
		if(total_num > TIME_WINDOW_SIZE)
		{
			...
			if(fabs(m_v_amcl[anchor_name].back().timestamp - time) < 1.0 && fabs(m_v_amcl[tag_name].back().timestamp - time) < 1.0)
			{
				...
				computeMeanVariance(v_odom_position_error,mean_odom_positioning_error,mean_odom_positioning_error);
				...
				cout << "cnt: " << error_cnt << "\n" 
					...
					<< "ODOM pose error: " << mean_odom_positioning_error << "\t" << mean_odom_positioning_error << "\n"
