...
int TIME_WINDOW_SIZE = 800;
int TIME_GRAP = 40;
const int NUM_POINTS = 20;
const int LAMDA = 20;
const double UPDATE_DIST = 0.01;  // m
const double UPDATE_ANGLE = 0.01; // m
const double UPDATE_TIME = 0.5;  // s
const double FINAL_TIME_WINDOW_SIZE = 1000;
...
{  
	...
	g2o::SE3Quat rel_se3;
	g2o::SE3Quat odom_se3;
	rel_se3 = computePoseToG2oSE3(relative_pose);
	odom_se3 = computePoseToG2oSE3(odom_pose.anchorPose);
...
double uwbKalmanFilter(UWBMessage uwb_message, CommonPose now_odom, RobotPose est_rel, KalmanFilter *kf, std::string name)
{
	...
	{   
		...
		g2o::SE3Quat T2 = computePoseToG2oSE3(m_old_odom_of_uwb[name].anchorPose).inverse() * computePoseToG2oSE3(now_odom.anchorPose);
		g2o::SE3Quat T1 = computePoseToG2oSE3(m_old_odom_of_uwb[name].tagPose).inverse() * computePoseToG2oSE3(now_odom.tagPose); 	
		g2o::SE3Quat now_pose_se3 = T2.inverse() * (rel_se3 * T1);
		rel_ = computeG2oSE3ToPose(now_pose_se3);
		dist_pre = sqrt((rel_.x * rel_.x) + (rel_.y * rel_.y));
	}
...
void* computePositionError(void* args)
{
	...
	{	
		...
		for(int i=0; i<v_robot1_amcl.size(); i++)
		{
			...
			tag_se3 = computePoseToG2oSE3(amcl.tagPose);
			rel_amcl_se3 = anchor_se3.inverse() * tag_se3;
			rel_amcl = computeG2oSE3ToPose(rel_amcl_se3);
...
std::map<std::string, int> m_old_size;
void* robot12_relative_position(void* args)
{
	while(ros::ok())
	{
		std::string anchor_name("robot1");
		std::string tag_name("robot2");

		if(m_v_com_odom.find(anchor_name+tag_name) == m_v_com_odom.end())
			continue;

		int total_num = m_v_com_odom[anchor_name+tag_name].size();
		if(m_old_size[anchor_name+tag_name] == total_num)
			continue;
		m_old_size[anchor_name+tag_name] = total_num;
		...
		{
			relative_pose = nonlinearOptimization_process(total_num, anchor_name, tag_name, m_v_com_odom[anchor_name+tag_name], v_robot12_uwb_measure);
			
			nav_msgs::Odometry rel_pose;
			rel_pose.pose.pose.orientation.x = relative_pose.timestamp;
			rel_pose.pose.pose.position.x = relative_pose.x;
			rel_pose.pose.pose.position.y = relative_pose.y;
			rel_pose.pose.pose.position.z= relative_pose.yaw;
			robot2_pub.publish(rel_pose);
		...
		// cout << "g2o_time_cast: " << g2o_time_cast << endl; 
	}
}
void* robot13_relative_position(void* args)
{
	while(ros::ok())
	{
		std::string anchor_name("robot1");
		std::string tag_name("robot3");
		...
		if(total_num > TIME_WINDOW_SIZE)
		{
			relative_pose = nonlinearOptimization_process(total_num, anchor_name, tag_name, m_v_com_odom[anchor_name+tag_name], v_robot13_uwb_measure);
...
int main(int argc, char *argv[]){
	...
	pthread_t tid1;
	int ret1 = pthread_create(&tid1, NULL, robot12_relative_position, NULL);
	pthread_t tid2;
	int ret2 = pthread_create(&tid2, NULL, robot13_relative_position, NULL);
	ros::spin();
	return 0;
}
