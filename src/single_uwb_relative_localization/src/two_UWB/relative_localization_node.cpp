...
const int LAMDA = 20;
const double UPDATE_DIST = 0.2;  // m
const double UPDATE_ANGLE = 0.2; // rad
...
inline RobotPose computeT1inverse_x_T2(RobotPose T1, RobotPose T2)
{
	...
	relative_pose.y = (T2.y - T1.y - tan(T1.yaw)*(T2.x - T1.x))
						/ (deno_); 
	relative_pose.yaw = T2.yaw - T1.yaw;
...
inline RobotPose computeT1_x_T2(RobotPose T1, RobotPose T2)
{
	...
	relative_pose.yaw = T1.yaw+T2.yaw;
	return relative_pose;
}
...
boost::circular_buffer<double> v_win_dist(10);
class KalmanFilter{
public:
	KalmanFilter(){
		Xbar= 0;
						   ...
						   boost::circular_buffer<UWBMessage> &v_robot_uwb_measure)
{
	...
	{	
		...
		now_odom.anchorPose = m_v_odom[tag_name].back();
		now_odom.tagPose = m_v_odom[anchor_name].back();
		static int flag = -1;
		if(flag < 0)
		{
		...
		else
		{
			...
			if((delta_tag_dist < UPDATE_DIST) && (delta_tag_angle < UPDATE_ANGLE))
				return;
			m_old_odom_tag_acc_dist[anchor_name+tag_name] = now_odom.tagPose.accumulated_dist;
			m_old_odom_tag_acc_angle[anchor_name+tag_name] = now_odom.tagPose.accumulated_orientation;
		}
		...
		if(cnt <= 0)
			return;	
		UWBMessage sum;
		sum.range = sum.fpRssi = sum.rxRssi = 0;
		for(int i=0; i<cnt; i++)
		{
		...
		if(total_num1-TIME_WINDOW_SIZE > TIME_GRAP  && TIME_WINDOW_SIZE <= FINAL_TIME_WINDOW_SIZE)
		{
		...
		if(total_num > TIME_WINDOW_SIZE)
		{
			...
			// get rel error;
			if(fabs(m_v_amcl[anchor_name].back().timestamp - time) < 1.0)
			{
				static int error_cnt = 0;
				error_cnt++;
				RobotPose anchor_amcl = m_v_amcl[tag_name].back();
				RobotPose tag_amcl = m_v_amcl[anchor_name].back();
				...
				computeMeanVariance(v_rel_yaw_error,mean_yaw_error,std_dev_yaw_error);	
				...
				cout << "cnt: " << error_cnt << "\n" 
					...
					<< "pose error: " << mean_positioning_error << "\t" << std_dev_positioning_error << "\n"
					<< "yaw error: " << mean_yaw_error*180/3.14 << "\t" << std_dev_yaw_error*180/3.14 << "\n";
				...
				m_v_dis_opt_global_pose[anchor_name].emplace_back(global_pose_tag);
		...
		else if(fabs(m_v_amcl[anchor_name].back().timestamp - time) < 0.5)
		{
			RobotPose anchor_amcl = m_v_amcl[tag_name].back();
			RobotPose tag_amcl = m_v_amcl[anchor_name].back();
			double true_dist = sqrt(tag_amcl.x*tag_amcl.x+ tag_amcl.y*tag_amcl.y);
			cout << true_dist << "\t" << uwb_message.range << endl;
			// double dist_error = fabs(uwb_message.range - true_dist);
			double dist_error = fabs(uwb_message.range - true_dist);
			v_rel_uwb_error.emplace_back(dist_error);
			double mean_dist_error=0;
			double std_dev_dist_error=0;
			computeMeanVariance(v_rel_uwb_error, mean_dist_error, std_dev_dist_error);
			cout << "dist error: " << mean_dist_error << "\t" << std_dev_dist_error << "\n";

			std::ofstream file1(dis_dist_error_file, std::ios::app);
			file1 << to_string(time) << "\t" << to_string(dist_error) << "\n";
			file1.close();
			std::ofstream file2(dis_uwb_range_file, std::ios::app);
			file2 << to_string(time) << "\t" << to_string(true_dist) << "\n";
			file2.close();
		}
		if((fabs(m_v_amcl[anchor_name].back().timestamp - time) < 0.1))
		{
			// save com data
			CommonPose com_amcl;
			com_amcl.timeStamp = time;
			com_amcl.tagPose = m_v_amcl["robot1"].back();
			com_amcl.anchorPose = m_v_amcl["robot2"].back();
			CommonPose com_odom;
			com_odom.timeStamp = time;
			com_odom.tagPose = m_v_odom["robot1"].back();
			com_odom.anchorPose = m_v_odom["robot2"].back();
			UWBMessage uwb_dis = uwb_message;
			uwb_dis.timestamp = time;
			double true_dis = sqrt((com_amcl.tagPose.x-com_amcl.anchorPose.x)*(com_amcl.tagPose.x-com_amcl.anchorPose.x) 
			                       + (com_amcl.tagPose.y-com_amcl.anchorPose.y)*(com_amcl.tagPose.y-com_amcl.anchorPose.y));
			saveComData(com_odom, com_amcl, uwb_dis, true_dis);
		}
	}	
}
//========================================================================
...
#ifdef STATIC_UWB_EXP
	RobotPose pose1 = pose;
