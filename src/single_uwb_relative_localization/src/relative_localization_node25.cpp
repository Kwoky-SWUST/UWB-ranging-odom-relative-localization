// use filter combin odom and g2o
...
const int G2O_SIZE = 600;
const int Q_SIZE = 100;
bool D_F_G_flag = false;
const int UP_L = 30;
const int DOWN_L = 30;
// #define DATA_FILTER_G2O
...
pf2::ParticleFilter6D *pf_odom1;
pf2::ParticleFilter6D *pf_odom2;
pf2::ParticleFilter6D *pf_odom3;
int m_num_samples = 100;
double m_init_range_uncertainty = 0.5;
double m_init_yaw_uncertainty = 0.3;

double m_odom_motion_noise=0.1;
double m_odom_cross_motion_noise=0.1;
double m_odom_orientation_noise=0.1;
double m_odom_theta_d_noise=0.5;
...
double to_y_w = 0.6;
double to_yaw_w = 0.1;
std::mutex m_mtx_pf_est;
bool m_pf_odom1 = false;
bool m_pf_odom2 = false;
bool m_pf_odom3 = false;
//------------------------------------------------------------------------
...
std::string dis_amcl_file("/home/dzy/Desktop/paper/dis_amcl_file.txt");
std::string dis_odom_file("/home/dzy/Desktop/paper/dis_odom_file.txt");
std::string dis_uwb_range_file("/home/dzy/Desktop/paper/dis_uwb_range_file.txt");
std::string dis_error_file("/home/dzy/Desktop/paper/dis_error_file.txt");
std::string dis_dist_error_file("/home/dzy/Desktop/paper/dis_dist_error_file.txt");
std::string dis_dist_std_file("/home/dzy/Desktop/paper/dis_dist_std_file.txt");
std::string dis_rxfx_std_file("/home/dzy/Desktop/paper/dis_rxfx_std_file.txt");
std::string dis_rxfx_file("/home/dzy/Desktop/paper/dis_rxfx_file.txt");
std::string dis_fx_file("/home/dzy/Desktop/paper/dis_fx_file.txt");
std::string dis_op_file("/home/dzy/Desktop/paper/dis_op_file.txt");
std::string dis_raw_file("/home/dzy/Desktop/paper/dis_raw_file.txt");

std::string all_pose_g2o_file("/home/dzy/Desktop/大论文实验数据/exp3/all_pose_g2o.txt");
std::string all_pose_amcl_file("/home/dzy/Desktop/大论文实验数据/exp3/all_pose_amcl.txt");
std::string all_pose_odom_file("/home/dzy/Desktop/大论文实验数据/exp3/all_pose_odom.txt");
// g2o
...
void plotUWBRel(int minX=-8, int maxX=15, int minY=-8, int maxY=20)
{
	...
	for(it_amcl=m_v_dis_opt_global_pose.begin(); it_amcl!=m_v_dis_opt_global_pose.end(); it_amcl++)
    {
        std::string line_colour; 
        std::string line_type;
        std::string line_name;
...
std::map <std::string, std::vector<RobotPose> >::iterator it_amcl;
	...
	// 	cmd += "e\n";
	// }
	std::ofstream file(all_pose_g2o_file, std::ios::out);
	for(it = m_g2o_results.begin(); it != m_g2o_results.end(); ++it)
	{
		...
		for(int i=0;i<trajectories_low_frequency.size();i=i+1)
		{
			...
			file << it->first << "\t" << trajectories_low_frequency[i].x << "\t" << trajectories_low_frequency[i].y << "\t" << trajectories_low_frequency[i].yaw << "\n";
		}
		cmd += "e\n";
	}
	file.close();
...
GnuplotInterface * m_plot_fp = new GnuplotInterface();
void plotfp(int minX=0, int maxX=650, int minY=0, int maxY=15)
{
	std::string cmd;
    ...
    cmd+="'-' u 1:2 w l lw 2 pt 1 ps 1 lc rgb '#FF0000' ti 'fp',";
    cmd += "\n";
	
    for(int i=0;i<v_dis_fx.size();i=i+1)
    {
        cmd += to_string( v_dis_time[i]) + ' ' + to_string( v_dis_fx[i] ) + ' ' + to_string( 0.5 ) + '\n';
    }
    cmd += "e\n";
    m_plot_fp->commandStr( cmd );
}

GnuplotInterface * m_plot_fprxstd = new GnuplotInterface();
void plotfprxstd(int minX=0, int maxX=650, int minY=0, int maxY=15)
{
	std::string cmd;
    ...
    cmd+="'-' u 1:2 w l lw 2 pt 1 ps 1 lc rgb '#FF0000' ti 'fprxstd',";
    cmd += "\n";
	
    for(int i=0;i<v_dis_rxfx_std.size();i=i+1)
    {
        cmd += to_string( v_dis_time[i]) + ' ' + to_string( v_dis_rxfx_std[i] ) + ' ' + to_string( 0.5 ) + '\n';
    }
    cmd += "e\n";
    m_plot_fprxstd->commandStr( cmd );
}
GnuplotInterface * m_plot_dist_std = new GnuplotInterface();
void plotdiststd(int minX=0, int maxX=650, int minY=0, int maxY=10)
{
	std::string cmd;
    ...
    cmd+="'-' u 1:2 w l lw 2 pt 1 ps 1 lc rgb '#FF0000' ti 'diststd',";
    cmd += "\n";
	
    for(int i=0;i<v_dis_dist_std.size();i=i+1)
    {
        cmd += to_string( v_dis_time[i]) + ' ' + to_string( v_dis_dist_std[i] ) + ' ' + to_string( 0.5 ) + '\n';
    }
    cmd += "e\n";
    m_plot_dist_std->commandStr( cmd );
}
GnuplotInterface * m_plot_fp_rx = new GnuplotInterface();
void plotfprx(int minX=0, int maxX=650, int minY=0, int maxY=15)
{
	std::string cmd;
    ...
    cmd+="'-' u 1:2 w l lw 2 pt 1 ps 1 lc rgb '#FF0000' ti 'fprx',";
    cmd += "\n";
	
    for(int i=0;i<v_dis_rxfx.size();i=i+1)
    {
        cmd += to_string( v_dis_time[i]) + ' ' + to_string( v_dis_rxfx[i] ) + ' ' + to_string( 0.5 ) + '\n';
    }
    cmd += "e\n";
    m_plot_fp_rx->commandStr( cmd );
}
GnuplotInterface * m_plot_rawuwb = new GnuplotInterface();
void plotrawuwb(int minX=0, int maxX=650, int minY=0, int maxY=15)
{
	std::string cmd;
    ...
    cmd+="'-' u 1:2 w l lw 2 pt 1 ps 1 lc rgb '#FF0000' ti 'raw',";
    cmd += "\n";
	
    for(int i=0;i<v_dis_raw_uwb.size();i=i+1)
    {
        cmd += to_string( v_dis_time[i]) + ' ' + to_string( v_dis_raw_uwb[i] ) + ' ' + to_string( 0.5 ) + '\n';
    }
    cmd += "e\n";
    m_plot_rawuwb->commandStr( cmd );
}
GnuplotInterface * m_plot_opuwb = new GnuplotInterface();
void plotopuwb(int minX=0, int maxX=650, int minY=0, int maxY=15)
{
	std::string cmd;
    ...
    cmd+="'-' u 1:2 w l lw 2 pt 1 ps 1 lc rgb '#FF0000' ti 'op',";
    cmd += "\n";
	
    for(int i=0;i<v_dis_op_uwb.size();i=i+1)
    {
        cmd += to_string( v_dis_time[i]) + ' ' + to_string( v_dis_op_uwb[i] ) + ' ' + to_string( 0.5 ) + '\n';
    }
    cmd += "e\n";
    m_plot_opuwb->commandStr( cmd );
}
//---------------------------------------------------------------------------
...
double uwbKalmanFilter(UWBMessage uwb_message, CommonPose now_odom, RobotPose est_rel, KalmanFilter *kf, std::string name)
{
	...
	bool NLOS_flag;
	v_dis_raw_uwb.emplace_back(uwb_message.range);
	v_dis_time.emplace_back(uwb_message.timestamp);
	std::ofstream file5(dis_raw_file , std::ios::app);
	file5 << to_string(uwb_message.timestamp) << "\t" << to_string(uwb_message.range) << "\n";
	file5.close();
	// Current
	...
	v_dis_rxfx_std.push_back(std_rx_fp);
	v_dis_fx.push_back(fp);
	std::ofstream file1(dis_rxfx_std_file, std::ios::app);
	file1 << to_string(uwb_message.timestamp) << "\t" << to_string(std_rx_fp) << "\n";
	file1.close();

	std::ofstream file2(dis_fx_file, std::ios::app);
	file2 << to_string(uwb_message.timestamp) << "\t" << to_string(fp) << "\n";
	file2.close();
#endif
#ifdef DIS_MOTHED 
	v_win_dist.push_back(uwb_message.range);
	double sum=0;
	for(int i=0; i<v_win_dist.size(); i++)
	{
		sum += v_win_dist[i];
	}  
	double mean_dist = sum/(double)(v_win_dist.size());
	sum=0;
	for(int i=0; i<v_win_dist.size(); i++)
	{
		sum += (v_win_dist[i]-mean_dist)* (v_win_dist[i]-mean_dist);
	}
	double std_dist =   sqrt(sum/(double)(v_win_dist.size()));
	...
	v_dis_dist_std.emplace_back(std_dist);
	std::ofstream file3(dis_dist_std_file , std::ios::app);
	file3 << to_string(uwb_message.timestamp) << "\t" << to_string(std_dist) << "\n";
	file3.close();
#endif
#ifdef SUB_MOTHED 
	if(rx_fp>6)
	...
	v_dis_rxfx.emplace_back(rx_fp);
	std::ofstream file4(dis_rxfx_file , std::ios::app);
	file4 << to_string(uwb_message.timestamp) << "\t" << to_string(rx_fp) << "\n";
	file4.close();
#endif
	...
	double op_dist = kf->Kalman_filters(uwb_message.range, NLOS_flag, dist_pre);
	v_dis_op_uwb.emplace_back(op_dist);	
	std::ofstream file6(dis_op_file , std::ios::app);
	file6 << to_string(uwb_message.timestamp) << "\t" << to_string(op_dist) << "\n";
	file6.close();
	return 	op_dist;
}
...
std::queue<RobotPose> drone1_odom_pose;
std::queue<RobotPose> drone2_odom;
std::queue<RobotPose> drone3_odom;
std::queue<RobotPose> drone2_pose;
std::queue<RobotPose> drone3_pose;
std::queue<RobotPose> drone12_pose;
std::queue<RobotPose> drone13_pose;
std::queue<RobotPose> drone23_pose;
#ifdef  DATA_FILTER_G2O
	bool compare (int a, int b)
	{
		return a<b;
	}
#endif
std::queue< std::map<std::string, RobotPose> > drone_rel_pose;
...
void saveG2oData(double time, RobotPose est_res, CommonPose odom, std::string anchor_name,  std::string tag_name)
{
	...
	{
		RobotPose tag_pose = computeG2oSE3ToPose(computePoseToG2oSE3(odom.anchorPose)*computePoseToG2oSE3(est_res));
		...
		if(tag_name == "robot2")
		{
			...
			drone1_odom_pose.push(odom.anchorPose);
			drone2_odom.push(odom.tagPose);
			drone2_pose.push(tag_pose);
			if(drone1_odom_pose.size() > G2O_SIZE)
				drone1_odom_pose.pop();
			if(drone2_odom.size() > G2O_SIZE)
				drone2_odom.pop();
			if(drone2_pose.size() > G2O_SIZE)
				drone2_pose.pop();

			// m_mtx_pose_est.lock();
			// m_v_robot_pose_g2o["drone1"].push_back(odom.anchorPose);
			// m_v_odom_pose_g2o["drone2"].push_back(odom.tagPose);
			// m_mtx_pose_est.unlock();
		}
		else if(tag_name == "robot3")
		{
			drone3_odom.push(odom.tagPose);
			if(drone3_odom.size() > G2O_SIZE)
				drone3_odom.pop();
			drone3_pose.push(tag_pose);
			if(drone3_pose.size() > G2O_SIZE)
				drone3_pose.pop();

			name_tag = "drone3";
			// m_mtx_pose_est.lock();
			// m_v_odom_pose_g2o["drone3"].push_back(odom.tagPose);
			// m_mtx_pose_est.unlock();
		}
#ifdef SAVE_G2O_DATA
...
#endif
		// m_mtx_pose_est.lock();
		// m_v_robot_pose_g2o[name_tag].push_back(tag_pose);
		// m_mtx_pose_est.unlock();
	}
	if(anchor_name == "robot1" && tag_name == "robot2")
	{
		...
		drone12_pose.push(est_res);
		if(drone12_pose.size() > G2O_SIZE)
			drone12_pose.pop();
		
		// m_mtx_pose_est.lock();
		// m_v_rel_pose_g2o["drone1"].g2o_estimate["drone2"].push_back(est_res);
		// m_mtx_pose_est.unlock();
	}
	if(anchor_name == "robot1" && tag_name == "robot3")
	{
		...
		drone13_pose.push(est_res);
		if(drone13_pose.size() > G2O_SIZE)
			drone13_pose.pop();

		// m_mtx_pose_est.lock();
		// m_v_rel_pose_g2o["drone1"].g2o_estimate["drone3"].push_back(est_res);
		// m_mtx_pose_est.unlock();
	}
	if(anchor_name == "robot2" && tag_name == "robot3")
	{
		...
		drone23_pose.push(est_res);
		if(drone23_pose.size() > G2O_SIZE)
			drone23_pose.pop();

		// m_mtx_pose_est.lock();
		// m_v_rel_pose_g2o["drone2"].g2o_estimate["drone3"].push_back(est_res);
		// m_mtx_pose_est.unlock();
	}

	// get robot1
	m_mtx_pose_est.lock();
	m_v_odom_pose_g2o.clear();
	m_v_robot_pose_g2o.clear();
	m_v_rel_pose_g2o.clear();
	m_mtx_pose_est.unlock();

	// std::cout << "size: " << drone1_odom_pose.size() << "\t" << drone2_pose.size() << "\t" << drone3_pose.size() << "\n"
	// 		  <<  drone2_odom.size() << "\t" << drone3_odom.size() << "\n"
	// 		  << drone12_pose.size() << "\t" << drone13_pose.size() << "\t" << drone23_pose.size() << "\n";
#ifdef  DATA_FILTER_G2O
	int D12_INDEX[Q_SIZE]={0};
	int D3_INDEX[Q_SIZE]={0};
	ros::Time be = ros::Time::now();
	if(drone1_odom_pose.size() > Q_SIZE && drone3_pose.size() > Q_SIZE)
	{
		int D12 = drone1_odom_pose.size(); 
		int D3 = drone3_pose.size();
		// search rand point
		for(int i=0; i<Q_SIZE; i++)
		{
			...
			else if(i > Q_SIZE-UP_L)
			{
				D12_INDEX[i] = ((D12 -1) - UP_L) + (i - (Q_SIZE-UP_L));
			}
			else
			{
				int r = carmen_uniform_random(DOWN_L,D12-UP_L);
				bool flag = false;
				for(int j = 0; j<i; j++)
				{
					if(r == D12_INDEX[j])
		...
		sort(D12_INDEX, D12_INDEX+Q_SIZE, compare);
		...
		for(int i=0; i<Q_SIZE; i++)
		{
			...
			{
				D3_INDEX[i] = ((D3 -1) - UP_L) + (i - (Q_SIZE-UP_L));
			}
			else
			{
				int r = carmen_uniform_random(DOWN_L,D3-UP_L);
				bool flag = false;
				for(int j = 0; j<i; j++)
				{
					if(r == D3_INDEX[j])
		...
		sort(D3_INDEX, D3_INDEX+Q_SIZE, compare);
		ros::Time en = ros::Time::now();
		// cout << en.toSec() - be.toSec() << endl;
		for(int i=0; i<Q_SIZE; i++)
			std::cout << D3_INDEX[i] << "\t" << D12_INDEX[i] << std::endl;
		D_F_G_flag = true;
	}
	int cnt_drone1 = 0;
	int cnt_drone2 = 0;
	...
	int cnt_drone12 = 0;
#endif
	for(int i=0; i<drone1_odom_pose.size(); i++)
	{
		RobotPose data;
		data = drone1_odom_pose.front();
		drone1_odom_pose.push(data);
		drone1_odom_pose.pop();
#ifdef  DATA_FILTER_G2O
		if(D_F_G_flag)
		{
			if(i == D12_INDEX[cnt_drone1])
			{
				m_mtx_pose_est.lock();
				m_v_robot_pose_g2o["drone1"].emplace_back(data);
				m_mtx_pose_est.unlock();
				cnt_drone1++;
				// cout << D12_INDEX[cnt_drone1] << endl;
			}
		}
		else
#endif
	...
	for(int i=0; i<drone2_pose.size(); i++)
	{
		RobotPose data;
		data = drone2_pose.front();
		drone2_pose.push(data);
		drone2_pose.pop();
#ifdef  DATA_FILTER_G2O
		if(D_F_G_flag)
		{
			if(i == D12_INDEX[cnt_drone2])
			{
				// cout << D12_INDEX[cnt_drone2] << endl;
				m_mtx_pose_est.lock();
				m_v_robot_pose_g2o["drone2"].emplace_back(data);
				m_mtx_pose_est.unlock();
				cnt_drone2++;
			}
		}
	...
	for(int i=0; i<drone3_pose.size(); i++)
	{
		RobotPose data;
		data = drone3_pose.front();
		drone3_pose.push(data);
		drone3_pose.pop();
#ifdef  DATA_FILTER_G2O
		if(D_F_G_flag)
		{
			if(i == D3_INDEX[cnt_drone3])
			{
				// cout << D3_INDEX[cnt_drone3] << endl;
				m_mtx_pose_est.lock();
				m_v_robot_pose_g2o["drone3"].emplace_back(data);
				m_mtx_pose_est.unlock();
				cnt_drone3++;
			}
		}
	...
	for(int i=0; i<drone2_odom.size(); i++)
	{
		RobotPose data;
		data = drone2_odom.front();
		drone2_odom.push(data);
		drone2_odom.pop();
#ifdef  DATA_FILTER_G2O
		if(D_F_G_flag)
		{
			if(i == D12_INDEX[cnt_odom2])
			{
				// cout << D12_INDEX[cnt_odom2] << endl;
				m_mtx_pose_est.lock();
				m_v_odom_pose_g2o["drone2"].emplace_back(data);
				m_mtx_pose_est.unlock();
				cnt_odom2++;
			}
		}
	...
	for(int i=0; i<drone3_odom.size(); i++)
	{
		RobotPose data;
		data = drone3_odom.front();
		drone3_odom.push(data);
		drone3_odom.pop();

#ifdef  DATA_FILTER_G2O
		if(D_F_G_flag)
		{
			if(i == D3_INDEX[cnt_odom3])
			{
				// cout << D3_INDEX[cnt_odom3] << endl;
				m_mtx_pose_est.lock();
				m_v_odom_pose_g2o["drone3"].emplace_back(data);
				m_mtx_pose_est.unlock();
				cnt_odom3++;
			}
		}
	...
	for(int i=0; i<drone12_pose.size(); i++)
	{
		RobotPose data;
		data = drone12_pose.front();
		drone12_pose.push(data);
		drone12_pose.pop();
#ifdef  DATA_FILTER_G2O
		if(D_F_G_flag)
		{
			if(i == D12_INDEX[cnt_drone12])
			{
				// cout << D3_INDEX[cnt_drone12] << endl;
				m_mtx_pose_est.lock();
				m_v_rel_pose_g2o["drone1"].g2o_estimate["drone2"].emplace_back(data);
				m_mtx_pose_est.unlock();
				cnt_drone12++;
			}
		}
	...
	for(int i=0; i<drone13_pose.size(); i++)
	{
		RobotPose data;
		data = drone13_pose.front();
		drone13_pose.push(data);
		drone13_pose.pop();
		m_mtx_pose_est.lock();
		m_v_rel_pose_g2o["drone1"].g2o_estimate["drone3"].emplace_back(data);
		m_mtx_pose_est.unlock();
	}
	for(int i=0; i<drone23_pose.size(); i++)
	{
		RobotPose data;
		data = drone23_pose.front();
		drone23_pose.push(data);
		drone23_pose.pop();
		m_mtx_pose_est.lock();
		m_v_rel_pose_g2o["drone2"].g2o_estimate["drone3"].emplace_back(data);
		m_mtx_pose_est.unlock();
	}
						   ...
						   boost::circular_buffer<UWBMessage> &v_robot_uwb_measure)
{
	...
	{	
		...
		if(total_num > TIME_WINDOW_SIZE)
		{
			...
			if(anchor_name == "robot1")
			{
				RobotPose anchor_amcl = now_odom.anchorPose;
				g2o::SE3Quat global_tag_se3 = computePoseToG2oSE3(anchor_amcl)*computePoseToG2oSE3(relative_pose);
				RobotPose global_pose_tag = computeG2oSE3ToPose(global_tag_se3);
				m_v_dis_opt_global_pose[tag_name].emplace_back(global_pose_tag);
				m_v_dis_opt_global_pose[anchor_name].emplace_back(anchor_amcl);
			}
#else			
...
#ifndef NO_AMCL
			if(fabs(m_v_amcl[anchor_name].back().timestamp - time) < 1.0 && fabs(m_v_amcl[tag_name].back().timestamp - time) < 1.0)
			{
				static int error_cnt = 0;
				error_cnt++;
				RobotPose anchor_amcl = m_v_amcl[anchor_name].back();
				RobotPose tag_amcl = m_v_amcl[tag_name].back();
				g2o::SE3Quat rel_amcl_se3 = computePoseToG2oSE3(anchor_amcl).inverse()*computePoseToG2oSE3(tag_amcl);

				// g2o::SE3Quat rel_error_se3 = rel_amcl_se3.inverse() * computePoseToG2oSE3(relative_pose);
				// RobotPose rel_error = computeG2oSE3ToPose(rel_error_se3);
				// double yaw_error = fabs(carmen_normalize_theta(rel_error.yaw));
				// double position_error = sqrt(rel_error.x*rel_error.x + rel_error.y*rel_error.y);

				// v_rel_yaw_error.emplace_back(yaw_error);
				// v_rel_position_error.emplace_back(position_error);

				// double mean_positioning_error=0;
				// double std_dev_positioning_error=0;


				// double mean_uwb_yaw_error=0;
				// double std_dev_yaw_error=0;

				// computeMeanVariance(v_rel_position_error,mean_positioning_error,std_dev_positioning_error);
				// computeMeanVariance(v_rel_yaw_error,mean_uwb_yaw_error,std_dev_yaw_error);	

				// double true_dist = sqrt((tag_amcl.x-anchor_amcl.x)*(tag_amcl.x-anchor_amcl.x) + (tag_amcl.y-anchor_amcl.y)*(tag_amcl.y-anchor_amcl.y));
				// double dist_error = fabs(uwb_message.range - true_dist);
				// v_rel_uwb_error.emplace_back(dist_error);
				// double mean_dist_error=0;
				// double std_dev_dist_error=0;
				// computeMeanVariance(v_rel_uwb_error, mean_dist_error, std_dev_dist_error);
				
				g2o::SE3Quat odom_se3 = computePoseToG2oSE3(now_odom.anchorPose).inverse() * computePoseToG2oSE3(now_odom.tagPose);
				g2o::SE3Quat odom_error_se3 = rel_amcl_se3.inverse() * odom_se3;
				RobotPose odom_error = computeG2oSE3ToPose(odom_error_se3);
				double odom_yaw_error = fabs(carmen_normalize_theta(odom_error.yaw));
				double odom_position_error = sqrt(odom_error.x*odom_error.x + odom_error.y*odom_error.y);

				v_odom_yaw_error.emplace_back(odom_yaw_error);
				v_odom_position_error.emplace_back(odom_position_error);

				double mean_odom_positioning_error=0;
				double std_dev_odom_positioning_error=0;


				double mean_odom_yaw_error=0;
				double std_dev_odom_yaw_error=0;

				computeMeanVariance(v_odom_position_error,mean_odom_positioning_error,std_dev_odom_positioning_error);
				computeMeanVariance(v_odom_yaw_error,mean_odom_yaw_error,std_dev_odom_yaw_error);

				cout << "cnt: " << error_cnt << "\n" 
					// << "dist error: " << mean_dist_error << "\t" << std_dev_dist_error << "\n"
					// << "UWB pose error: " << mean_positioning_error << "\t" << std_dev_positioning_error << "\n"
					// << "UWB yaw error: " << mean_uwb_yaw_error*180/3.14 << "\t" << std_dev_yaw_error*180/3.14 << "\n"
					<< "ODOM pose error: " << mean_odom_positioning_error << "\t" << std_dev_odom_positioning_error << "\n"
					<< "ODOM yaw error: " << mean_odom_yaw_error*180/3.14 << "\t" << std_dev_odom_yaw_error*180/3.14 << "\n" ;
				
				// push all data to display
				// g2o::SE3Quat global_tag_se3 = computePoseToG2oSE3(anchor_amcl)*computePoseToG2oSE3(relative_pose);
				// RobotPose global_pose_tag = computeG2oSE3ToPose(global_tag_se3);
				// m_v_dis_opt_global_pose[tag_name].emplace_back(global_pose_tag);
				
				// std::ofstream file1(dis_dist_error_file, std::ios::app);
				// file1 << to_string(time) << "\t" << to_string(dist_error) << "\n";
				// file1.close();
				
				// std::ofstream file2(dis_error_file, std::ios::app);
				// file2 << to_string(time) << "\t" << to_string(position_error) << "\t" << to_string(yaw_error*180/3.14) << "\n";
				// file2.close();

				// std::ofstream file3(dis_amcl_file, std::ios::app);
				// file3 << to_string(time) << "\t" << to_string(anchor_amcl.x) << "\t" << to_string(anchor_amcl.y) << "\t" << to_string(anchor_amcl.yaw) << "\t"
				//       << to_string(tag_amcl.x) << "\t" << to_string(tag_amcl.y) << "\t" << to_string(tag_amcl.yaw) << "\n";
				// file3.close();
				// std::ofstream file4(dis_odom_file, std::ios::app);
				// file4 << to_string(time) << "\t" << to_string(global_pose_tag.x) << "\t" << to_string(global_pose_tag.y) << "\t" << to_string(global_pose_tag.yaw) << "\n";
				// file4.close();
				// std::ofstream file5(dis_uwb_range_file, std::ios::app);
				// file5 << to_string(time) << "\t" << to_string(true_dist) << "\n";
				// file5.close();

				// formation 
				// if(anchor_name == "robot1")
				// {
				// 	RobotPose goal = m_goal[tag_name];
				// 	g2o::SE3Quat f_error_se3 = computePoseToG2oSE3(relative_pose).inverse() * computePoseToG2oSE3(goal);
				// 	RobotPose f_error = computeG2oSE3ToPose(f_error_se3);
				// 	double e_theta = f_error.yaw;
				// 	double e_x = f_error.x;
				// 	double e_y = f_error.y;
				// 	std::cout << "formation_error: ex: " << e_x << "\t" << "\te_y: " << e_y << "\te_theta: " << e_theta << std::endl; 
				// }
			}
#endif
	}	
...
void updateParticleWeights_g2o(RobotPose g2o_pose, pf2::ParticleFilter6D *pf)
{

	double temp_pf_weight[m_num_samples];
	for(int i=0; i<m_num_samples;i++)
		temp_pf_weight[i]=1.0;
	pf2::ParticleFilter6D *pf_odom = pf;
	// Pose odom_mean;
	// double odom_covariance[36];
	// pf_odom->computeUnweightedMeanAndCovariance(odom_mean, odom_covariance);

	RobotPose g2o_odom = g2o_pose;
	for(int u1=0;u1<pf_odom->size();u1++)
	{
		double weight=1.0;
		pf2::Particle6D *particle_u1 = pf_odom->at(u1);

		double p_theta_odom=particle_u1->theta_z;
		double p_x_odom=particle_u1->x;
		double p_y_odom=particle_u1->y;

		double denominator=1.0/(2.0);
		double pre_factor=1.0/(0.2*1.5*1.5*sqrt(2*M_PI));

		double weight_to_mean=0;

		double g2o_x_odom = g2o_odom.x;
		double g2o_y_odom = g2o_odom.y;
		double g2o_theta_odom = g2o_odom.yaw;
		g2o_theta_odom = carmen_normalize_theta(g2o_theta_odom);

		double rela_angle = fabs(carmen_normalize_theta(p_theta_odom-g2o_theta_odom));
		double rela_dis = sqrt(p_x_odom*p_x_odom + p_y_odom*p_y_odom);
		double to_dis = sqrt(g2o_x_odom*g2o_x_odom + g2o_y_odom*g2o_y_odom);
		double to_to = (rela_dis-to_dis)*(rela_dis-to_dis)/(to_to_w*to_to_w);
		
		double to_x = (p_x_odom-g2o_x_odom)*(p_x_odom-g2o_x_odom)/(to_x_w*to_x_w);
		double to_y = (p_y_odom-g2o_y_odom)*(p_y_odom-g2o_y_odom)/(to_y_w*to_y_w);
		double to_theta = (rela_angle)*(rela_angle)/(to_yaw_w*to_yaw_w);

		
		weight_to_mean=exp( -denominator*(to_x + to_y + to_theta));
		weight_to_mean=pow(weight_to_mean, 1.0/3.0);
	...
	for(int u1=0;u1<pf_odom->size();u1++)
	{
		...
		double weights = pow(temp_pf_weight[u1], 1.0/2.0);
		if(weights<m_minLikelihood)
		{
			weights=m_minLikelihood;
		}
		else if(weights>m_maxLikelihood)
		{
			weights=m_maxLikelihood;
		}
		particle_u1->weight *= weights;
	}
	...
	//resample if needed
	if (pf_odom->getEffectiveSampleSize() < pf_odom->size()/2 )
	{
		//	std::cout<<"resample"<<std::endl;
		pf_odom->resample();	
	}

	// pf_odom->computeUnweightedMeanAndCovariance( mean, covariance);
}

void robot1OdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
	ros::Time odom_beg = ros::Time::now();
	...
	if(get_odom1_flag)
	{
		est_current_odom1 = est_current_odom1_g2o;
		get_odom1_flag = 0;
	}
	m_mtx_odom_est.unlock();
#endif
	...
	else
	{
		...
		double actual_delta_d = sigma_trans;
		double sigma_rot1;
		...
		{
			m_mtx_pf_est.lock();
			pf2::ParticleFilter6D * uav_pf = pf_odom1;
			for(int u=0;u<uav_pf->size();u++)
			{
				...
				RobotPose cre_odom;
				RobotPose diff;
				RobotPose new_particles_d;
				RobotPose cur_pf;
				cur_pf.x = current_particle->x;
				cur_pf.y = current_particle->y;
				cur_pf.yaw = current_particle->theta_z;
				pre_odom = robot1_previous_odom; 
				cre_odom.x = current_x;
				cre_odom.y = current_y;
				cre_odom.yaw = current_theta;
				diff = computeG2oSE3ToPose(computePoseToG2oSE3(pre_odom).inverse() * computePoseToG2oSE3(cre_odom));
				new_particles_d = computeG2oSE3ToPose(computePoseToG2oSE3(cur_pf) * computePoseToG2oSE3(diff));

				double delta_x=actual_delta_x;
				double delta_y=actual_delta_y;
				double delta_d=sqrt(delta_x*delta_x+delta_y*delta_y);

				double xx_noise=delta_d*cos(current_particle->theta_z)*carmen_gaussian_random(0,m_odom_motion_noise);
				double yy_noise=delta_d*sin(current_particle->theta_z)*carmen_gaussian_random(0,m_odom_motion_noise);

				double xy_cross_noise=delta_d*sin(current_particle->theta_z)*carmen_gaussian_random(0,m_odom_cross_motion_noise);
				double yx_cross_noise=delta_d*cos(current_particle->theta_z)*carmen_gaussian_random(0,m_odom_cross_motion_noise);

				double theta_to_theta_noise=fabs(actual_delta_theta)*carmen_gaussian_random(0,m_odom_orientation_noise);

				double theta_to_d_noise=delta_d*carmen_gaussian_random(0,m_odom_theta_d_noise);


				current_particle->x=new_particles_d.x +xx_noise+xy_cross_noise;
				current_particle->y=new_particles_d.y +yy_noise+yx_cross_noise;
				current_particle->z=msg->pose.pose.position.z;
				current_particle->theta_x=roll;
				current_particle->theta_y=pitch;
			...
			pf_est_odom.x = mean.x;
			pf_est_odom.y = mean.y;
			pf_est_odom.yaw = mean.theta_z;
			m_mtx_pf_est.unlock();
		}
#endif
	...
	if(!m_pf_odom1)
		pf_est_odom =pose;
	m_v_est_globoal_pose["robot1"].push_back(pf_est_odom);
#else
	...
	est_pose_.header.frame_id = "robot1/odom";
	est_pose_.pose.pose.position.x = pf_est_odom.x;
	est_pose_.pose.pose.position.y = pf_est_odom.y;
	est_pose_.pose.pose.position.z = pf_est_odom.yaw;
	est_pose_.pose.pose.orientation.x = pose.accumulated_dist;
	robot1_pub.publish(est_pose_);
	ros::Time odom_end = ros::Time::now();
	// cout << "odom_time_cost:" << odom_end.toSec() - odom_beg.toSec() << endl;
#else
	nav_msgs::Odometry est_pose_;
	est_pose_.header.stamp = msg->header.stamp;
	est_pose_.header.frame_id = "robot1/odom";
...
#endif
#ifdef SAVE_G2O_DATA
	std::ofstream amcl_file(drone_odom_file1, std::ios::app);
	amcl_file << "drone1" << "\t" << to_string(pose.timestamp) << "\t" << to_string(pose.x) << "\t" << to_string(pose.y) << "\t" << to_string(pose.yaw) << "\n";
	amcl_file.close();
#endif
...
void robot2OdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
	...
	if(get_odom2_flag)
	{
		est_current_odom2 = est_current_odom2_g2o;
		get_odom2_flag = 0;
	}
	m_mtx_odom_est.unlock();
#endif
	...
	else
	{
		...
		if(m_pf_odom2)
		{
			m_mtx_pf_est.lock();
			double actual_delta_d = sigma_trans;
			pf2::ParticleFilter6D * uav_pf = pf_odom2;
			for(int u=0;u<uav_pf->size();u++)
			{
				pf2::Particle6D *current_particle = uav_pf->at(u);
	...
	if(!m_pf_odom2)
		pf_est_odom = pose;
	m_v_est_globoal_pose["robot2"].push_back(pf_est_odom);
#else
	est_pose.timestamp = timestamp;
	...
	nav_msgs::Odometry est_pose_;
	est_pose_.header.stamp = msg->header.stamp;
	est_pose_.header.frame_id = "robot2/odom";
	est_pose_.pose.pose.position.x = pf_est_odom.x;
	est_pose_.pose.pose.position.y = pf_est_odom.y;
	est_pose_.pose.pose.position.z= pf_est_odom.yaw;
	est_pose_.pose.pose.orientation.x = pose.accumulated_dist;
...
#endif
#ifdef SAVE_G2O_DATA
	std::ofstream amcl_file(drone_odom_file2, std::ios::app);
	amcl_file << "drone2" << "\t" << to_string(pose.timestamp) << "\t" << to_string(pose.x) << "\t" << to_string(pose.y) << "\t" << to_string(pose.yaw) << "\n";
	amcl_file.close();
#endif
}
//------------------------------------------------------------------------
void robot3OdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
	...
	if(get_odom3_flag)
	{
		est_current_odom3 = est_current_odom3_g2o;
		get_odom3_flag = 0;
	}
	m_mtx_odom_est.unlock();
#endif
	...
	else
	{
		...
		if(m_pf_odom3)
		{
			m_mtx_pf_est.lock();
			double actual_delta_d = sigma_trans;
			pf2::ParticleFilter6D * uav_pf = pf_odom3;
			for(int u=0;u<uav_pf->size();u++)
			{
				pf2::Particle6D *current_particle = uav_pf->at(u);
	...
	if(!m_pf_odom3)
		pf_est_odom = pose;
	m_v_est_globoal_pose["robot3"].push_back(pf_est_odom);
#else
	est_pose.timestamp = timestamp;
	...
	nav_msgs::Odometry est_pose_;
	est_pose_.header.stamp = msg->header.stamp;
	est_pose_.header.frame_id = "robot3/odom";
	est_pose_.pose.pose.position.x = pf_est_odom.x;
	est_pose_.pose.pose.position.y = pf_est_odom.y;
	est_pose_.pose.pose.position.z= pf_est_odom.yaw;
	est_pose_.pose.pose.orientation.x = pose.accumulated_dist;
...
#endif
#ifdef SAVE_G2O_DATA
	std::ofstream amcl_file(drone_odom_file3, std::ios::app);
	amcl_file << "drone3" << "\t" << to_string(pose.timestamp) << "\t" << to_string(pose.x) << "\t" << to_string(pose.y) << "\t" << to_string(pose.yaw) << "\n";
	amcl_file.close();
#endif
}
//------------------------------------------------------------------------
...
void* g2o_process(void* args)
{
	while(1)
	{
		//cout << m_v_odom_pose.size() << "\t" << m_v_robot_pose.size() <<  "\t" << m_v_rel_pose.size() <<endl;
		m_mtx_pose_est.lock();
		std::map< std::string, std::vector<RobotPose> > m_v_odom_pose = m_v_odom_pose_g2o;
		std::map< std::string, ObjectConfig > m_v_rel_pose = m_v_rel_pose_g2o;
		std::map< std::string, std::vector<RobotPose> > m_v_robot_pose = m_v_robot_pose_g2o;
		m_mtx_pose_est.unlock();
		...
		{
			RobotPose g2o_pose;
			g2o_pose.x =  record_data["drone1"][robot1_lab].x;
			g2o_pose.y =  record_data["drone1"][robot1_lab].y;
			g2o_pose.yaw =  carmen_normalize_theta(record_data["drone1"][robot1_lab].theta_z);
#ifdef USE_PF 
			ros::Time pf_b = ros::Time::now();
			static int flag = 1;
			if(flag)
			{
				flag = 0;
				...
				double covariance[36];
				m_mtx_pf_est.lock();
				pf_odom1 = new pf2::ParticleFilter6D();
				pf2::ParticleFilter6D *pf = pf_odom1;
				pf2::Particle6D p;
				for ( int i = 0; i < m_num_samples; ++i )
				{

					p.x=carmen_uniform_random(g2o_pose.x-m_init_range_uncertainty,g2o_pose.x+m_init_range_uncertainty);
					p.y = carmen_uniform_random(g2o_pose.y-m_init_range_uncertainty,g2o_pose.y+m_init_range_uncertainty);
					p.z = 0;
					p.theta_z = carmen_uniform_random(g2o_pose.yaw-m_init_yaw_uncertainty,g2o_pose.yaw+m_init_yaw_uncertainty);
					p.theta_x = 0;
					p.theta_y = 0;
			...
			{
				m_mtx_pf_est.lock();
				updateParticleWeights_g2o(g2o_pose, pf_odom1);
				m_mtx_pf_est.unlock();
			}
			ros::Time pf_e = ros::Time::now();
			// cout << pf_e.toSec()-pf_b.toSec() << endl; 
#else
			m_mtx_odom_est.lock();
			get_odom1_flag = 1;
			est_current_odom1_g2o = g2o_pose;
			m_mtx_odom_est.unlock();
#endif			
			
#ifdef PUB_G2O	
			nav_msgs::Odometry est_pose_;
			est_pose_.header.stamp = ros::Time::now();
			est_pose_.pose.pose.position.x = record_data["drone1"][robot1_lab].x;
			est_pose_.pose.pose.position.y = record_data["drone1"][robot1_lab].y;
			est_pose_.pose.pose.position.z = carmen_normalize_theta(record_data["drone1"][robot1_lab].theta_z);
			robot1_pub.publish(est_pose_);
#endif
		}
		...
		{
			RobotPose g2o_pose;
			g2o_pose.x =  record_data["drone2"][robot2_lab].x;
			g2o_pose.y =  record_data["drone2"][robot2_lab].y;
			g2o_pose.yaw =  carmen_normalize_theta(record_data["drone2"][robot2_lab].theta_z);
#ifdef USE_PF 
			static int flag = 1;
			if(flag)
			{
				...
				double covariance[36];
				m_mtx_pf_est.lock();
				pf_odom2 = new pf2::ParticleFilter6D();
				pf2::ParticleFilter6D *pf = pf_odom2;
				pf2::Particle6D p;
				for ( int i = 0; i < m_num_samples; ++i )
				{
			...
			{
				m_mtx_pf_est.lock();
				updateParticleWeights_g2o(g2o_pose, pf_odom2);
				m_mtx_pf_est.unlock();
			}
#else
			m_mtx_odom_est.lock();
			get_odom2_flag = 1;
			est_current_odom2_g2o = g2o_pose;
			m_mtx_odom_est.unlock();
#endif		
#ifdef PUB_G2O	
			nav_msgs::Odometry est_pose_;
			est_pose_.header.stamp = ros::Time::now();
			est_pose_.pose.pose.position.x = record_data["drone2"][robot2_lab].x;
			est_pose_.pose.pose.position.y = record_data["drone2"][robot2_lab].y;
			est_pose_.pose.pose.position.z = carmen_normalize_theta(record_data["drone2"][robot2_lab].theta_z);
			robot2_pub.publish(est_pose_);
#endif
		}
		...
		{
			RobotPose g2o_pose;
			g2o_pose.x =  record_data["drone3"][robot3_lab].x;
			g2o_pose.y =  record_data["drone3"][robot3_lab].y;
			g2o_pose.yaw =  carmen_normalize_theta(record_data["drone3"][robot3_lab].theta_z);
#ifdef USE_PF 
			static int flag = 1;
			if(flag)
			{
				...
				m_mtx_pf_est.lock();
				pf_odom3 = new pf2::ParticleFilter6D();
				pf2::ParticleFilter6D *pf = pf_odom3;
				pf2::Particle6D p;
				for ( int i = 0; i < m_num_samples; ++i )
				{
			...
			{
				m_mtx_pf_est.lock();
				updateParticleWeights_g2o(g2o_pose, pf_odom3);
				m_mtx_pf_est.unlock();
			}
#else
			m_mtx_odom_est.lock();
			get_odom3_flag = 1;
			est_current_odom3_g2o = g2o_pose;
			m_mtx_odom_est.unlock();
#endif		
#ifdef PUB_G2O	
			nav_msgs::Odometry est_pose_;
			est_pose_.header.stamp = ros::Time::now();
			est_pose_.pose.pose.position.x = record_data["drone3"][robot3_lab].x;
			est_pose_.pose.pose.position.y = record_data["drone3"][robot3_lab].y;
			est_pose_.pose.pose.position.z = carmen_normalize_theta(record_data["drone3"][robot3_lab].theta_z);
			robot3_pub.publish(est_pose_);
#endif
		}
		...
		m_g2o_results.clear();
		ros::Time end_g2o = ros::Time::now();
		std::cout << end_g2o - begin_g2o << std::endl;
		// sleep(1);
	}
}
...
int main(int argc, char *argv[]){
	...
	std::ofstream file51(dis_amcl_file, std::ios::out);
	std::ofstream file52(dis_odom_file, std::ios::out);
	std::ofstream file53(dis_uwb_range_file, std::ios::out);
	std::ofstream file54(dis_error_file, std::ios::out);
	std::ofstream file55(dis_dist_error_file, std::ios::out);
	std::ofstream file56(dis_dist_std_file, std::ios::out);
	std::ofstream file57(dis_rxfx_std_file, std::ios::out);
	std::ofstream file58(dis_rxfx_file, std::ios::out);
	std::ofstream file59(dis_fx_file, std::ios::out);
	std::ofstream file60(dis_op_file, std::ios::out);
	std::ofstream file61(dis_raw_file, std::ios::out);

	std::ofstream file62(all_pose_amcl_file, std::ios::out);
	std::ofstream file63(all_pose_odom_file, std::ios::out);
	...
	goal.x = INITIAL2_X;
	goal.y = INITIAL2_Y;
	goal.yaw = INITIAL2_YAW;
	m_goal["robot2"] = goal;
	goal.x = INITIAL3_X;
	goal.y = INITIAL3_Y;
	goal.yaw = INITIAL3_YAW;
	m_goal["robot3"] = goal;
	...
	robot1_pub = n.advertise<nav_msgs::Odometry>("/robot1/est_odom", 1);
	robot2_pub = n.advertise<nav_msgs::Odometry>("/robot2/est_odom", 1);
	robot3_pub = n.advertise<nav_msgs::Odometry>("/robot3/est_odom", 1);
