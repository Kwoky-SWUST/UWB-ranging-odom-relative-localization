...
private:
    ...
    float Delta_Angle(float a1, float a2); // delta
    int Bisect_Angle(int angle1, int angle2); // 平均角度

    bool Cant_Turn_To_Goal();

    // Returns 0 if something got inside the safety distance, else 1.
    int Calculate_Cells_Mag( double laser_ranges[REQUIRED_NUM_RANGES][2], int speed ); // 计算单元值
    // Returns 0 if something got inside the safety distance, else 1.
    int Build_Primary_Polar_Histogram( double laser_ranges[REQUIRED_NUM_RANGES][2], int speed ); // 建立一维极坐标直方图
    int Build_Binary_Polar_Histogram(int speed); // 二值化图
    int Build_Masked_Polar_Histogram(int speed); // 
    int Select_Candidate_Angle(); // 选择候选角度
    int Select_Direction(); // 选择方向
    int Set_Motion( int &speed, int &turnrate, int current_speed ); // 生成移动

    // Minimum turning radius at this speed, in millimeters
    int MinTurningRadius( int speed ); // 在当前速度下的转弯半径，mm

    void Print_Cells_Dir(); // 单元格方向
    void Print_Cells_Mag(); // 单元格值
    void Print_Cells_Dist(); // 单元格距离
    void Print_Cells_Sector(); // 单元格所在扇区
    void Print_Cells_Enlargement_Angle(); // 单元格扩大角度
    void Print_Hist(); // 直方图
