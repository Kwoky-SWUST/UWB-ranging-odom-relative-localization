...
int VFH_Algorithm::Init()
{
  ...
  CENTER_X = (int)floor(WINDOW_DIAMETER / 2.0);  //  窗口中心  
  CENTER_Y = CENTER_X;
  HIST_SIZE = (int)rint(360.0 / SECTOR_ANGLE); // 取整， 安扇区角度将窗口划分为N个直方图
  ...
  */
  ...
  VFH_Allocate(); // 创始化所有参数，给0值

  for(x=0;x<HIST_SIZE;x++) { //初始化直方图
    Hist[x] = 0;
  ...
  for(x=0;x<WINDOW_DIAMETER;x++) {
    for(y=0;y<WINDOW_DIAMETER;y++) {
      ...
      if (x < CENTER_X) {                                                            // -  | y
        if (y < CENTER_Y) {                                                          //    |
          Cell_Direction[x][y] = atan((float)(CENTER_Y - y) / (float)(CENTER_X - x));//---------x   
          Cell_Direction[x][y] *= (360.0 / 6.28);                                    //  /t|
