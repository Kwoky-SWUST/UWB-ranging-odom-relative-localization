...
void robot1PoseCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
 ...
 if(fabs(timestamp - m_v_global_pose["robot2"].back().timestamp) < 1)
 {
    ...
    if(r < detect_R && (fabs(robot21.x-obstacle2_pose.x) > L1 || fabs(robot21.y-obstacle2_pose.y) > L2) && (fabs(robot23.x-obstacle2_pose.x) > L1 || fabs(robot23.y-obstacle2_pose.y) > L2)) // 避障 
    {
 ...
 if(fabs(timestamp - m_v_global_pose["robot3"].back().timestamp) < 1)
 {
    ...
    std::cout << "diff2:" << d1 << "\t" << theta1  << std::endl;
    // 1.避障控制
    ...
    // std::cout << fabs(robot31.x-obstacle3_pose.x) << "\t" << fabs(robot31.y-obstacle3_pose.y) << std::endl;
    if(r < detect_R && (fabs(robot31.x-obstacle3_pose.x) > L1 || fabs(robot31.y-obstacle3_pose.y) > L2) && (fabs(robot32.x-obstacle3_pose.x) > L1 || fabs(robot32.y-obstacle3_pose.y) > L2)) // 避障 
    {
      geometry_msgs::Twist goal;
      goal.linear.x = d;
      goal.angular.z = theta;
